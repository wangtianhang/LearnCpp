...
namespace UnityEngine
{
    ...
    //因为前者就是计算向前向量到当前向量（1,0,0）的旋转量的，其实现过程就是后者喽。
        ...
        static Vector3 ToEulerRad(Quaternion q)
        {
            ...
            QuaternionToMatrix(q, ref m);
            MatrixToEuler(m, ref rot);
            return rot;
        }

        static void QuaternionToMatrix(Quaternion q, ref Matrix3x3 m)
        {
        ...
        static void SanitizeEuler(ref Vector3 euler)
        {
        ...
        static bool MatrixToEuler(Matrix3x3 matrix, ref Vector3 v)
        {
